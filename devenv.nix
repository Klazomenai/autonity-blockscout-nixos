...
in
{
  ...
  enterShell = ''
    ...
    echo ""
    echo "Nix:"
    echo "  nix flake check        Run flake checks (locally evaluates ALL: fmt + hardening + integration-sync)"
    echo "                         (CI runs only fmt+hardening on PR; full set on main + nightly)"
