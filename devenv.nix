...
in
{
  ...
  enterShell = ''
    ...
    echo ""
    echo "Nix:"
    echo "  nix flake check        Run flake checks (locally evaluates ALL: fmt + hardening + integration + integration-sync)"
    echo "                         (CI runs only fmt+hardening on PR; full set including both VM checks on main + nightly)"
