...
3. Firewall rules: 22/80/443/4000 verified via actual listener probe (`wait_for_open_port`); 30303/{tcp,udp} verified via `iptables-save` per-line inspection only — autonity runs `--nodiscover --maxpeers=0` in the VM so no p2p listener is bound, but the production firewall rule must still be present.
