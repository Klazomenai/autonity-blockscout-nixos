...
- **Internal networking**: UNIX sockets where the consuming application supports them at the URL/connection-string layer (Redis ✓ via Blockscout's Redix client; PostgreSQL ✗ because Blockscout's Postgrex layer parses URL host:port for the actual TCP connect and ignores libpq's `?host=` query for socket overrides — so PostgreSQL is TCP-localhost). Where UNIX sockets ARE used, cross-service access is granted via `SupplementaryGroups` on the consumer, never via `BindReadOnlyPaths` on the directory.
