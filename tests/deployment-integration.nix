...
pkgs.testers.nixosTest {
  ...
  testScript =
    ...
    # `throw` here aborts evaluation before the test script renders.
    # Wired as `if !ok then throw ... else <script>` rather than a
    # let-bound `_assertion` because deadnix recognises the
