{
  ...
  outputs =
    ...
    in
    {
      ...
      # Consumed by `nixos-anywhere --flake .#ovh-test` and by
      # `nixos-rebuild switch --flake .#ovh-test --target-host …`.
      #
      # Two host-specific files are merged in conditionally based on
      # `builtins.pathExists` so `nix flake check` evaluates cleanly
      # in CI (where neither exists):
      #
      #   - `./deployments/ovh-test/hardware-configuration.nix`
      #     Generated per-instance by
      #     `nixos-anywhere --generate-hardware-config` on first install.
      #     Gitignored.
      #
      #   - `./deployments/ovh-test/pharos-runtime.nix`
      #     Operator-rendered overlay containing `server_name` +
      #     `acme_email`. Generated by `make` from the operator's
      #     `~/.config/pharos-secrets/{server_name,acme_email}` files.
      #     Gitignored. Without it, the deployment falls back to the
      #     fail-closed `lib.mkDefault` placeholders in
      #     `configuration.nix` (`deployment.invalid` /
      #     `ops@deployment.invalid`).
      nixosConfigurations.ovh-test = nixpkgs.lib.nixosSystem {
        system = "x86_64-linux";
        modules = [
        ...
        ++ nixpkgs.lib.optional (builtins.pathExists ./deployments/ovh-test/hardware-configuration.nix) ./deployments/ovh-test/hardware-configuration.nix
        ++ nixpkgs.lib.optional (builtins.pathExists ./deployments/ovh-test/pharos-runtime.nix) ./deployments/ovh-test/pharos-runtime.nix;
      };
    }
